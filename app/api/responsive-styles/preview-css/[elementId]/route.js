...
export async function POST(request, { params }) {
  try {
    ...
    const css = manager.generateElementCSS(elementId);
    ...
    logger.error(`Error generating CSS preview: ${error.message}`, error);
    return NextResponse.json({ error: 'Failed to generate CSS preview' }, { status: 400 });
  }
}
